...
pub fn project() -> error::Result<&'static Box<dyn ProjectDirectory>> {
    static PROJECT: once_cell::sync::OnceCell<Box<dyn ProjectDirectory>> =
        once_cell::sync::OnceCell::new();

    PROJECT.get_or_try_init(|| {
        let project: Box<dyn ProjectDirectory> = if let Some(project) =
            directories::ProjectDirs::from("io.github", "holdxen", "SVNexus")
        {
...
impl dyn ProjectDirectory {
    pub fn database_file(&self) -> PathBuf {
        self.cache_directory().join("svnexus.surreal.db")
    }
