...
pub trait WcDifferenceCallback: Send + Sync + 'static {
    fn file_opened(
        ...
        path: String,
        revision: Option<u32>,
    ) -> error::Result<WcDifferenceFileOpenedResult, error::FrontendError>;
...
impl AsyncWorkingCopyContext {
    ...
    pub async fn get_replaced_file(&self, path: String) -> error::Result<Option<WcReplacedNode>> {
        ...
        unsafe extern "C" fn file_deleted(
            ...
            unsafe {
                let file = (baton as *mut Option<WcReplacedNode>)
                    .as_mut()
                ...
                if !file1.is_null() {
                    ...
                    let original_properties = if !original_properties.is_null() {
                        ...
                        Some(scratch_pool.convert_to_hash_map(original_properties))
                    } else {
                        None
                    };

                    *file = Some(WcReplacedNode::File {
                        content,
                        original_properties,
                    });
                } else {
        ...
        unsafe extern "C" fn file_opened(
            _tree_conflicted: *mut svn_boolean_t,
            _skip: *mut svn_boolean_t,
            _path: *const ::std::os::raw::c_char,
            _rev: svn_revnum_t,
            _diff_baton: *mut ::std::os::raw::c_void,
            _scratch_pool: *mut apr_pool_t,
        ) -> *mut svn_error_t {
            std::ptr::null_mut()
        }
        unsafe extern "C" fn file_changed(
            _contentstate: *mut svn_wc_notify_state_t,
            _propstate: *mut svn_wc_notify_state_t,
            _tree_conflicted: *mut svn_boolean_t,
            _path: *const ::std::os::raw::c_char,
            _tmpfile1: *const ::std::os::raw::c_char,
            _tmpfile2: *const ::std::os::raw::c_char,
            _rev1: svn_revnum_t,
            _rev2: svn_revnum_t,
            _mimetype1: *const ::std::os::raw::c_char,
            _mimetype2: *const ::std::os::raw::c_char,
            _propchanges: *const apr_array_header_t,
            _originalprops: *mut apr_hash_t,
            _diff_baton: *mut ::std::os::raw::c_void,
            _scratch_pool: *mut apr_pool_t,
        ) -> *mut svn_error_t {
            std::ptr::null_mut()
        ...
        unsafe extern "C" fn file_added(
            ...
            _mimetype1: *const ::std::os::raw::c_char,
            _mimetype2: *const ::std::os::raw::c_char,
            _copyfrom_path: *const ::std::os::raw::c_char,
            _copyfrom_revision: svn_revnum_t,
            _propchanges: *const apr_array_header_t,
            _originalprops: *mut apr_hash_t,
            _diff_baton: *mut ::std::os::raw::c_void,
            _scratch_pool: *mut apr_pool_t,
        ...
        unsafe extern "C" fn dir_deleted(
            _state: *mut svn_wc_notify_state_t,
            _tree_conflicted: *mut svn_boolean_t,
            ...
            unsafe {
                ...
                let file = (baton as *mut Option<WcReplacedNode>)
                    .as_mut()
                    .expect("Failed to cast baton to mutable reference");
                *file = Some(WcReplacedNode::Directory { path });
            }
            std::ptr::null_mut()
        }
        unsafe extern "C" fn dir_opened(
            _tree_conflicted: *mut svn_boolean_t,
            _skip: *mut svn_boolean_t,
            _skip_children: *mut svn_boolean_t,
            _path: *const ::std::os::raw::c_char,
            _rev: svn_revnum_t,
            _diff_baton: *mut ::std::os::raw::c_void,
            _scratch_pool: *mut apr_pool_t,
        ...
        unsafe extern "C" fn dir_props_changed(
            ...
            _tree_conflicted: *mut svn_boolean_t,
            _path: *const ::std::os::raw::c_char,
            _dir_was_added: svn_boolean_t,
            _propchanges: *const apr_array_header_t,
            _original_props: *mut apr_hash_t,
            _diff_baton: *mut ::std::os::raw::c_void,
            _scratch_pool: *mut apr_pool_t,
        ) -> *mut svn_error_t {
            std::ptr::null_mut()
        ...
        self.call_async(move |ctx| unsafe {
            ...
            let mut file: Option<WcReplacedNode> = None;
...
#[derive(PartialEq, Eq, PartialOrd, Ord, Clone, Copy, Debug, Serialize, Deserialize, ts_rs::TS)]
#[serde(rename_all = "camelCase")]
#[ts(export)]
...
impl From<*const ffi::svn_wc_conflict_version_t> for WorkingCopyConflictVersion {
    fn from(value: *const ffi::svn_wc_conflict_version_t) -> WorkingCopyConflictVersion {
        unsafe {
            let this = value.as_ref().expect("Failed to get reference to value");
...
pub struct WorkingCopyConflictDescription {
    ...
    source_left_version: Option<WorkingCopyConflictVersion>,
    source_right_version: Option<WorkingCopyConflictVersion>,
...
impl From<*const ffi::svn_wc_conflict_description2_t> for WorkingCopyConflictDescription {
    fn from(value: *const ffi::svn_wc_conflict_description2_t) -> Self {
        unsafe {
            ...
            let source_left_version = this
                .src_left_version
                .map(|p| WorkingCopyConflictVersion::from(p));
            let source_right_version = this
                .src_right_version
                .map(|p| WorkingCopyConflictVersion::from(p));
...
/*
 * conflict_func2 回调的合法返回值（svn_wc_conflict_choice_t）：
 *
 * choose_postpone
 *   推迟解决。冲突状态保留在工作副本中，用户后续可用 svn resolve 手动处理。
 *   适用于所有冲突类型（text / prop / tree）。
 *
 * choose_base
 *   使用共同祖先（base）版本覆盖当前文件。
 *   本地修改和 incoming 修改都会被丢弃。
 *   适用于 text / prop 冲突。
 *
 * choose_theirs_full
 *   完全使用 incoming 版本，丢弃所有本地修改。
 *   适用于 text / prop 冲突。
 *
 * choose_mine_full
 *   完全使用本地版本，丢弃所有 incoming 修改。
 *   适用于 text / prop 冲突。
 *
 * choose_theirs_conflict
 *   仅冲突的 hunk 使用 incoming 版本，非冲突区域保留自动合并结果。
 *   不能用于二进制文件或二进制属性（没有 hunk 概念）。
 *   适用于 text 冲突。
 *
 * choose_mine_conflict
 *   仅冲突的 hunk 使用本地版本，非冲突区域保留自动合并结果。
 *   不能用于二进制文件或二进制属性。
 *   适用于 text 冲突，以及 tree 冲突的特定场景（如 moved-away 节点）。
 *
 * choose_merged
 *   使用回调提供的自定义合并结果。
 *   需设置 result->merged_file（指向合并后的文件路径），
 *   属性冲突时也可设置 result->merged_value（优先级高于 merged_file）。
 *   tree 冲突时含义特殊：表示接受当前工作副本状态。
 *   适用于 text / prop / tree 冲突。
 *
 * 不应返回的值：
 *
 * choose_undefined
 *   内部使用，表示"尚未做出选择"，回调中不应返回。
 *
 * choose_unspecified
 *   调用方的控制标志，表示"没有预设方案，请逐个询问回调"。
 *   回调中返回此值会导致：
 *   - text 冲突：断言失败，程序崩溃
 *   - prop 冲突：被静默当作 postpone 处理
 *   - tree 冲突：返回错误
 */
...
pub struct WorkingCopyConflictResult {
    pub choice: WorkingCopyConflictChoice,
...
impl From<*const ffi::svn_wc_info_t> for WorkingCopyInfo {
    fn from(value: *const ffi::svn_wc_info_t) -> WorkingCopyInfo {
        unsafe {
            let ptr = value.as_ref().expect("Failed to get reference to value");
            let schedule = ptr.schedule.try_into().expect("Failed to convert schedule");
            ...
            let depth = ptr.depth.try_into().expect("Failed to convert depth");
            let recorded_size = ptr.recorded_size.try_into().ok();

            let recorded_time = ptr
                .recorded_time
                .try_into()
            ...
            let working_copy_format = ptr.wc_format.try_into().expect("Failed to convert format");
...
impl From<*const ffi::svn_merge_range_t> for MergeRange {
    fn from(ptr: *const ffi::svn_merge_range_t) -> Self {
        unsafe {
            ...
            Self {
                start: ptr.start.try_into().expect("Failed to convert hunk value"),
                end: ptr.end.try_into().expect("Failed to convert hunk value"),
...
impl From<*const ffi::svn_wc_notify_t> for WorkingCopyNotify {
    fn from(ptr: *const ffi::svn_wc_notify_t) -> Self {
        unsafe {
            ...
            let err = ptr.err.map(SubversionError::from_not_null_ptr);
            ...
            let hunk_original_start = ptr
                .hunk_original_start
                .try_into()
                .expect("Failed to convert hunk value");

            let hunk_original_length = ptr
                .hunk_original_length
                .try_into()
                .expect("Failed to convert size");

            let hunk_modified_start = ptr
                .hunk_modified_start
                .try_into()
                .expect("Failed to convert hunk value");

            let hunk_modified_length = ptr
                .hunk_modified_length
                .try_into()
                .expect("Failed to convert size");

            let hunk_matched_line = ptr
                .hunk_matched_line
                .try_into()
                .expect("Failed to convert hunk value");
...
#[derive(
    Debug, Copy, Clone, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize, ts_rs::TS,
)]
#[ts(export)]
...
impl From<*const ffi::svn_wc_revision_status_t> for WorkingCopyRevisionStatus {
    fn from(value: *const ffi::svn_wc_revision_status_t) -> Self {
        unsafe {
            ...
            let min_revision = value
                .min_rev
                .try_into()
                .expect("Failed to convert revision number");
            let max_revision = value
                .max_rev
                .try_into()
                .expect("Failed to convert revision number");
