...
use crate::{
    ...
    subversion::{
        context::{Depth, Lock, NodeKind, Revision, RevisionNumber},
        ffi, svn_no_error, SubversionError,
    },
    utils::{CStringer, Pointer, PointerMapper},
};
...
impl Context {
    pub fn list(&mut self, opts: ListOptions) -> error::Result<ListResult> {
        unsafe extern "C" fn list_receiver(
            ...
            unsafe {
                let entries = (baton as *mut Vec<ListEntry>)
                    .as_mut()
                ...
                if !dirent.is_null() {
                    let dirent = dirent.as_ref().expect("Failed to get reference to dirent");
                    kind = NodeKind::try_from(dirent.kind).ok();
                    size = dirent.size.try_into().ok();
                    has_properties = Some(dirent.has_props != 0);
                    created_revision = dirent.created_rev.try_into().ok();
                    time = Some(dirent.time);
                    last_author = dirent.last_author.to_nullable_string()
                }
        ...
        let mut entries: Vec<ListEntry> = Vec::with_capacity(128);
