...
impl StatusEntry {
    unsafe fn from_path_and_ptr(
        ...
        unsafe {
            ...
            let entry = entry.as_ref().expect("Failed to get reference to entry");

            let node_kind = NodeKind::try_from(entry.kind).expect("Unexpected failure");
            ...
            let file_size = if entry.filesize == -1 {
                ...
                Some(entry.filesize.try_into().expect("Failed to convert size"))
            };
            ...
            let depth = Depth::try_from(entry.depth).expect("Unexpected failure");

            let out_of_date_kind = NodeKind::try_from(entry.ood_kind).expect("Unexpected failure");

            let repository_node_status =
                WorkingCopyStatus::try_from(entry.repos_node_status).expect("Unexpected failure");

            let repository_text_status =
                WorkingCopyStatus::try_from(entry.repos_text_status).expect("Unexpected failure");

            let repository_property_status =
                WorkingCopyStatus::try_from(entry.repos_prop_status).expect("Unexpected failure");
...
impl StatusEntry {
    fn fix_node_kind(&mut self) -> error::Result<()> {
        ...
        let metadata = std::fs::symlink_metadata(self.path.as_str())?;
